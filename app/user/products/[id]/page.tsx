...
import { Category, Tenant, VariantOptionSchema, VariantOptionSet, Product } from '@/lib/types'
  ...
  const loadData = async (token: string, id: string) => {
    try {
      ...
      const [categoriesRes, variantSetsRes, productRes] = await Promise.all([
        ...
        fetch(`/api/products/${id}?subdomain=${encodeURIComponent(userTenant.subdomain)}`),
      ])
