...
export default function TenantAdminNewProductPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [categoriesRes, variantSetsRes] = await Promise.all([
        ...
        if (variantSetsRes.ok) {
          setVariantOptionSets(await variantSetsRes.json())
        }
      } catch (e) {
        console.error('Failed to load product dependencies', e)
      } finally {
        setInitialLoading(false)
      }
    }
    load()
  }, [tenant, loading, adminBase, router])
  ...
  if (loading || !tenant || initialLoading) {
    return (
    ...
    <div className="space-y-6">
      ...
      <Card className="max-w-3xl">
        <CardHeader>
          ...
          <CardDescription>Fill in product details, pricing, and variants.</CardDescription>
        </CardHeader>
        <CardContent>
                            ...
                            <td className="p-2">
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  next[idx] = {
                                    ...next[idx],
                                    stock: val === '' ? undefined : parseInt(val, 10),
                                  }
                                  setVariants(next)
                                }}
                                className="h-8 text-xs"
